...
func Equal(x, y interface{}) bool {
	if reflect.DeepEqual(x, y) != true {
		fmt.Printf("Expected : %v and Found %v !!", x, y)
		return false
	}
	return true
}
...
func TestChallengePacket(ot *testing.T) {
	...
	if Equal(err, nil) == false {
		ot.Fail()
	}
	p1out, ok := p1outI.(*ChallengePacket)

	if Equal(ok, true) == false {
		ot.Fail()
	}
	if Equal(*p1in, *p1out) == false {
		ot.Fail()
	}
...
// 	/*
// 		inByte := []byte{0x01, 0xED, 0x00, 0x1F, 0x10, 0x24, 0xDC, 0x3D, 0x6D, 0xB5, 0x66, 0xED, 0x25, 0xE4,
// 			0x90, 0x49, 0x2C, 0x6E, 0xA2, 0x65, 0xCD, 0x73, 0x74, 0x72, 0x6F, 0x6E, 0x67, 0x53, 0x77, 0x61, 0x6E}
// 		p2outI, err := Decode(inByte)
// 		Equal(err, nil)
// 		p2out, ok := p2outI.(*ChallengePacket)
// 		Equal(ok, true)
// 		Equal(p2out.Identifier, uint8(0xED))
// 		Equal(p2out.Challenge, [16]byte{0x24, 0xdc, 0x3d, 0x6d, 0xb5, 0x66, 0xed, 0x25, 0xe4, 0x90, 0x49, 0x2c, 0x6e, 0xa2, 0x65, 0xcd})
// 		Equal(p2out.Name, "strongSwan")
// 	*/
...
// 		ot.Fail()
// 	}
// 	if Equal(p2out.Identifier, uint8(0x22)) == false {
// 		ot.Fail()
// 	}
// 	if Equal(p2out.NTResponse, [24]uint8{0xa3, 0xc, 0x3a, 0xd4, 0xe9, 0x13, 0x69, 0xbf, 0x6e, 0x66, 0x6e, 0x13, 0xbd, 0x19, 0x7f, 0x5, 0xf0, 0xf2, 0x7a, 0x4d, 0xc2, 0x9a, 0xca, 0x17}) == false {
// 		ot.Fail()
// 	}

// 	if Equal(p2out.PeerChallenge, [16]uint8{0x84, 0xf5, 0xf7, 0xfd, 0xc8, 0x3a, 0x25, 0x22, 0x31, 0x18, 0x12, 0x17, 0x2, 0x8f, 0xa4, 0xf3}) == false {
// 		ot.Fail()
// 	}
// 	if Equal(p2out.Name, "Q6VgtqdU4XOW8rJp") == false {
// 		ot.Fail()
// 	}
...
// 	if Equal(p3out.Identifier, uint8(0x22)) == false {
// 		ot.Fail()
// 	}

// 	if Equal(p3out.Auth, [20]byte{0x46, 0xa5, 0xc0, 0xfa, 0xc5, 0x9d, 0xca, 0xc1, 0xef, 0xaa, 0x9e, 0x1f, 0xeb, 0x76, 0x60,
// 		0xb6, 0xba, 0x30, 0x8d, 0x39}) == false {
// 		ot.Fail()
// 	}

// 	if Equal(p3out.Message, "Welcome2strongSwan\x00") == false {
// 		ot.Fail()
// 	}
// 	if Equal(p3out.Encode(), inByte3) == false {
// 		ot.Fail()
// 	}
...
// 	})
// 	if Equal(p3outget.Encode(), inByte3) == false {
// 		ot.Fail()
// 	}
